...
func waitForTCP(peerManager IPeerManager, listener net.Listener, initiatorConn chan ConnAndType) {
	...
	for {
		...
		goUtils.HandleErr(err, "Parsing IP")
		var senderIP []byte
		...
		if !peerManager.IsConnected(senderIPString) {
			if msgType[0] == 0 {
				...
				goUtils.HandleErr(err, "Error while reading message ")
				...
				goUtils.HandleErr(err, "Error while connecting to sender")
				if newConn == nil {
				...
				for k := 2; k < len(peerInfo); k += 6 {
					...
					if !peerManager.IsConnected(peerIP.String()) {
						...
						goUtils.HandleErr(err, "Error while connecting to peer")
